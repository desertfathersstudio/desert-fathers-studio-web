...
// ── Data fetching ──────────────────────────────────────────────────────
async function fetchOrder(sessionId: string): Promise<OrderData | null> {
  // 1. Try Supabase — should exist after webhook fires
  try {
    ...
    if (data) {
      return {
        ...
        items: (data.retail_order_items ?? []).map((i: {
  ...
  // 2. Stripe fallback — catches webhook race condition window
  try {
    ...
    const orderNumber = `DFS-${sessionId.replace(/^cs_(test_|live_)?/, "").slice(0, 8).toUpperCase()}`;
    ...
    let shippingAddr: Record<string, string | null | undefined> | null = null;
  ...
  return (
    <div className="min-h-screen flex items-center justify-center px-4" style={{ background: "var(--bg)" }}>
      <div className="text-center max-w-sm">
        ...
        <h1 className="font-serif text-2xl font-semibold mb-2" style={{ color: "var(--text)" }}>
...
function SuccessView({ order }: { order: OrderData }) {
  ...
  const addrLine = [addr?.line1, addr?.city, addr?.state, addr?.postal_code].filter(Boolean).join(", ");
    ...
    <div style={{ background: "var(--bg)", minHeight: "100svh" }}>
      ...
      <div className="max-w-lg mx-auto px-6 py-12">
        <div className="text-center mb-10">
          ...
          <h1 className="font-serif text-3xl font-semibold mb-2" style={{ color: "var(--text)" }}>
            Order Confirmed
          </h1>
        ...
        >
          ...
          <div className="px-6 py-5">
            <h2 className="text-xs font-medium uppercase tracking-wide mb-4" style={{ color: "var(--text-muted)" }}>
              Items
            </h2>
                ...
                <li key={idx} className="flex items-center gap-3">
                  {item.filename && (
                    <div
                      ...
                      style={{ width: 48, height: 48, border: "1px solid var(--border)", background: "#fff" }}
                    >
                      <Image
                        src={stickerImageUrl(item.filename)}
                        alt={item.productName} fill
                        className="object-contain p-1" sizes="48px"
                      />
                    </div>
                  )}
                  ...
                  <span className="text-sm tabular-nums shrink-0" style={{ color: "var(--text)" }}>
          ...
          {addrLine && (
            <>
              <div style={{ borderTop: "1px solid var(--border)" }} />
              <div className="px-6 py-5">
                <h2 className="text-xs font-medium uppercase tracking-wide mb-2" style={{ color: "var(--text-muted)" }}>
                  Shipping to
                </h2>
        ...
        <div
          ...
          style={{ background: "rgba(107,31,42,0.05)", border: "1px solid rgba(107,31,42,0.12)" }}
        >
