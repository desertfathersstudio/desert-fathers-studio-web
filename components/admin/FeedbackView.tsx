...
// ── Shared product list (used in both desktop sidebar and mobile view) ──────
...
function ProductList({
  ...
  selected: ProductWithComments | null;
  onSelect: (p: ProductWithComments) => void;
}) {
  return (
    <>
      <div
      ...
      {products.map((product) => {
        ...
        return (
          ...
          >
            ...
            {/* Chevron hint */}
            <span style={{ fontSize: "1.1rem", color: isActive ? "rgba(255,255,255,0.4)" : C.textMuted, flexShrink: 0 }}>
              ›
            </span>
          </button>
        );
...
export function FeedbackView() {
  ...
  function handleSelect(p: ProductWithComments) {
    setSelected(p);
  ...
  return (
    <>
      {/* ── DESKTOP: fixed-height two-panel side-by-side ── */}
      <div
        className="hidden md:flex"
        style={{ height: "calc(100dvh - 52px)", overflow: "hidden" }}
      >
        {/* Left: list */}
        <aside
          style={{
        ...
        >
          <ProductList products={products} selected={selected} onSelect={handleSelect} />
        </aside>
      ...
      <div
        ...
        style={{ background: "#faf6ef", minHeight: "100%" }}
      >
        <ProductList products={products} selected={selected} onSelect={handleSelect} />
      </div>

      {/* ── MOBILE: full-screen detail overlay (position: fixed) ── */}
      {mobileDetailOpen && selected && (
        <div
          className="fixed inset-0 z-50 flex flex-col md:hidden"
          style={{ background: C.contentBg, overflowY: "auto" }}
        >
          <ProductDetail
            product={selected}
            onRefresh={fetchData}
          ...
          >
            ...
            <div style={{ padding: "1rem 1.125rem" }}>
              ...
              <div style={{ display: "flex", gap: "0.5rem", flexWrap: "wrap" }}>
                <ActionBtn onClick={() => handleResolve(parent)} loading={busy === parent.id + "-resolve"} variant={parent.is_resolved ? "ghost" : "resolve"}>
                ...
                <ActionBtn onClick={() => handleDelete(parent.id)} loading={busy === parent.id + "-delete"} variant="danger">
                  Delete
                </ActionBtn>
              </div>

              {replyingTo === parent.id && (
                <div style={{ marginTop: "0.875rem", padding: "0.875rem", background: C.contentBg, borderRadius: 8, border: `1px solid ${C.border}`, display: "flex", flexDirection: "column", gap: "0.625rem" }}>
                  <textarea
                    autoFocus
                    value={replyText}
                  ...
                  <div style={{ display: "flex", gap: "0.5rem" }}>
                    ...
                    <ActionBtn onClick={() => setReplyingTo(null)} variant="ghost">Cancel</ActionBtn>
                  </div>
                </div>
              )}
            </div>
