    ...
    <div>
      ...
      <div style={{ display: "flex", gap: "1rem", marginBottom: "1.5rem", flexWrap: "wrap" }}>
        {[
          ...
          { label: "Active", value: orders.filter((o) => !["Delivered"].includes(o.orderStage)).length },
          { label: "Payment Awaiting Confirmation", value: pendingPayment, highlight: pendingPayment > 0 },
        ].map((s) => (
          <div
            key={s.label}
            style={{
              background: s.highlight ? "#fff3cd" : "var(--bg-card, #f5f0e8)",
              border: `1px solid ${s.highlight ? "#ffc107" : "var(--border, #e4d8c8)"}`,
              borderRadius: 10,
          ...
          {(["All", ...ORDER_STAGES] as (OrderStage | "All")[]).map((s) => (
            <button
              ...
              style={{
                ...
                color: stageFilter === s ? "#fff" : "inherit",
                fontSize: "0.72rem",
      ...
      {filtered.length === 0 ? (
        <div style={{ padding: "3rem", textAlign: "center", color: "var(--text-muted, #7a6a5a)", fontSize: "0.88rem" }}>
          No orders found.
        </div>
      ) : (
...
function AdminOrderCard({
  order, expanded, onToggle, onUpdated,
}: {
  ...
  return (
    <div
      style={{
        ...
        borderLeft: `4px solid ${order.asap ? "#e65100" : order.paymentSent && !order.paymentReceived ? "#ffc107" : "var(--brand, #6B1F2A)"}`,
        overflow: "hidden",
      }}
    >
      <button
      ...
      >
        <div style={{ display: "flex", alignItems: "center", gap: "0.6rem", flexWrap: "wrap", flex: 1, minWidth: 0 }}>
          ...
          {order.paymentReceived && (
            <span style={{ fontSize: "0.68rem", fontWeight: 600, padding: "2px 8px", borderRadius: "999px", background: "#d4edda", color: "#155724" }}>✓ Payment Received</span>
          )}
        </div>
      ...
      {expanded && (
        <div style={{ padding: "0 1.25rem 1.25rem", borderTop: "1px solid var(--border, #e4d8c8)" }}>
          {/* Stage tracker */}
          <div style={{ display: "flex", alignItems: "flex-start", margin: "1rem 0 1rem", gap: 0 }}>
          ...
          <div style={{ borderTop: "1px solid var(--border, #e4d8c8)", paddingTop: "1rem", display: "flex", flexDirection: "column", gap: "0.75rem" }}>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: "0.5rem", flexWrap: "wrap" }}>
              ...
              {order.paymentSentDate && (
                <span style={{ fontSize: "0.72rem", color: "var(--text-muted, #7a6a5a)" }}>{order.paymentSentDate}</span>
              )}
              {order.paymentSent && (
                <button
                  ...
                  style={{
                    ...
                    background: order.paymentReceived ? "white" : "#155724",
                    color: order.paymentReceived ? "#155724" : "white",
                    border: order.paymentReceived ? "1px solid #155724" : "none",
                    borderRadius: 6, fontSize: "0.74rem", fontWeight: 600, cursor: "pointer",
                  }}
                >
